...
def get_classifier(args, trial):
    ...
    train_dataset, valid_dataset, test_dataset = dataset_creator.get_datasets(path, trial_path, trial, transform, unk_in_valid=True, gallery=True)
    ...
    # Model
    file_name = f"./models/arcface_{trial}_{args.name}.pth"
    ...
    encoder.load_state_dict(torch.load(file_name, weights_only=True, map_location=device))
